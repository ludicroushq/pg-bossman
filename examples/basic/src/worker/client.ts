...
export async function sendWelcomeEmail(to: string, name: string) {
  const jobId = await client.client.emails.sendWelcome.send({ name, to });
...
export async function requestPasswordReset(email: string) {
  ...
  const jobId = await client.client.emails.sendPasswordReset.send({
    to: email,
    token,
  });
...
export async function resizeImages(
  ...
  const jobIds = await Promise.all(
    images.map((img) => client.client.media.resizeImage.send(img))
  );
...
export async function scheduleCleanup(path: string, daysOld: number) {
  const jobId = await client.client.maintenance.cleanupOldFiles.send({
    daysOld,
    path,
  });
...
export async function processPayment(
  ...
  const jobId = await client.client.payments.processPayment.send({
    amount,
    currency,
    customerId,
    orderId,
