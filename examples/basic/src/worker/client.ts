...
export async function sendWelcomeEmail(to: string, name: string) {
  const jobId = await client.queues.sendWelcomeEmail.send({ name, to });
...
export async function requestPasswordReset(email: string) {
  ...
  const jobId = await client.queues.sendPasswordResetEmail.send({
    to: email,
    token,
  });
...
export async function resizeImages(
  ...
  const jobIds = await Promise.all(
    images.map((img) => client.queues.resizeImage.send(img))
  );
...
export async function scheduleCleanup(path: string, daysOld: number) {
  const jobId = await client.queues.cleanupOldFiles.send({
    daysOld,
    path,
  });
...
export async function processPayment(
  ...
  const jobId = await client.queues.processPayment.send({
    amount,
    currency,
    customerId,
    orderId,
